...
############################################
# Datadog Monitoring (toggleable)
############################################

# Create namespace only when enabled
...
# Secret holding the Datadog API key (managed by Terraform state)
...
# Install Datadog via Helm when enabled
resource "helm_release" "datadog" {
  count            = var.datadog_enable ? 1 : 0
  name             = "datadog"
  repository       = "https://helm.datadoghq.com"
  chart            = "datadog"
  version          = "~> 3.71" # pin to a recent stable chart
  namespace        = var.datadog_namespace
  ...
  # Enable kube-state-metrics core (via Cluster Agent)
  set {
  ...
  # Orchestrator Explorer toggle
  set {
    name  = "orchestratorExplorer.enabled"
    value = tostring(var.datadog_enable_orchestrator)
  }

  # Logs/APM/Process toggles
  set {
    name  = "datadog.logs.enabled"
    value = tostring(var.datadog_enable_logs)
  }
  set {
    name  = "datadog.apm.enabled"
    value = tostring(var.datadog_enable_apm)
  }
  set {
    name  = "datadog.processAgent.enabled"
    value = tostring(var.datadog_enable_process)
  }
